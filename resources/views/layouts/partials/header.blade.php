<header class="bg-white border-b border-slate-200 px-6 py-3 flex items-center justify-between gap-4">
    <div class="flex items-center gap-4 flex-1 min-w-0">
        <a href="{{ route('dashboard') }}" class="lg:hidden shrink-0">
            <x-wantex-logo size="sm" :tagline="false" />
        </a>
        <div class="flex-1 max-w-md">
            <div class="relative">
                ...
                <input type="search" placeholder="Rechercher..." class="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-pink-500">
            </div>
        </div>
    </div>
        ...
        <x-dropdown align="right" width="48">
            <x-slot name="trigger">
                <button class="flex items-center gap-2 text-sm text-slate-700 hover:text-slate-900">
                    <div class="w-8 h-8 rounded-full bg-pink-700 text-white flex items-center justify-center text-xs font-semibold">
